...
regexTime = re.compile('^[0-9]{2}:[0-9]{2}:[0-9]{2}$')
...
def fromStr2Time(string,default = [0,0,0]):
   
   if regexTime.match(string): # If a time is given correct format HH:MM:SS
      [hours,minutes,seconds] = map(int, string.split(':'))
   else:
      print("Errore il formato inserito non e' corretto")
      [hours,minutes,seconds] = default

   return Timer(hour=hours, min=minutes, sec=seconds)
...
class Timer(object):
   ...
   def copy(self): # Return a copy of the object
